...
$difference_water = 0;
$Water_total = 0;
$water_cost_per_room = 0; // เพิ่มตัวแปรนี้
...
function calculateWaterCost($room_number) {
    ...
    // ค่าน้ำพื้นฐาน
    $basic_water_cost = 100; // ค่าเริ่มต้นสำหรับห้องอื่น ๆ
...
function saveBill($conn, $room_number, $Electricity_total, $Water_total, $price, $total) {
    ...
    $total_with_water = $total + $water_cost; // รวมค่าน้ำพื้นฐานในยอดรวม
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['room_number'])) {
        ...
        $water_cost_per_room = calculateWaterCost($room_number); // กำหนดค่าน้ำพื้นฐานที่นี่
    }
    ...
    if (isset($_POST['calculate'])) {
        ...
        if ($current_electric < $last_month_electric || $current_electric < 0) {
            ...
            $total = $Electricity_total + $price + $water_cost_per_room; // รวมค่าน้ำพื้นฐานในยอดรวม
    ...
    <div class="card">
        ...
        <table>
            ...
            <tbody>
                ...
                <?php } ?>
                <tr>
                    <td>ค่าน้ำประจำห้อง</td>
                    <td><?php echo htmlspecialchars($water_cost_per_room); ?> บาท</td>
                </tr>
                <tr>
                    <td>ค่าห้องพัก</td>
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" style="margin-top: 1rem;">
            ...
            <input type="hidden" name="water_cost_per_room" value="<?php echo htmlspecialchars($water_cost_per_room); ?>">
